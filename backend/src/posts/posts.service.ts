...
export class PostsService {
  ...
  async getInitialPosts(fetchCount: number, userObjectID?: string): Promise<PostDto[]> {
    ...
    // if every item from postDocuments is over a week old, also throw new NotFoundException
    // let anyIsFresh = false;
    // for (const doc of postDocuments) {
    //   if (this.isOverOneWeekOld(doc)) {
    //     anyIsFresh = true;
    //     break;
    //   }
    // }

    // if (!anyIsFresh && !(user && initialFetchOffset < user.posts.length)) {
    //   Logger.log("returning 404 because all found posts are too old", "getInitialPosts")
    //   throw new NotFoundException()
    // }
   ...
   */
  async getInitialPostsOld(fetchCount: number, userObjectID?: string): Promise<PostDto[]> {
