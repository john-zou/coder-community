...
describe('Comments Helper Functions', () => {
  ...
  it('createComment -- invalid post ID', async () => {
    ...
    let err;
    try {
      await createComment(postAuthor._id.toString(), {commentRoot: CommentRoot.POST.toString(), content: "test content", parentPost: postID});
    } catch (error) {
      err = error;
    }
  ...
  it('createComment reply to a post', async () => {
    ...
    const post = await createTestPost(postAuthor);
    const postID = post._id.toString();
    const actual = await createComment(postAuthor._id.toString(), {commentRoot: CommentRoot.POST.toString(), content: "test content", parentPost: postID});
    const comments = await CommentModel.find();
  ...
  it('createComment reply to a comment of a post', async () => {
    ...
    const post = await createTestPost(postAuthor);
    const postID = post._id.toString();
    const parentCommentDto = await createComment(postAuthor._id.toString(), {commentRoot: CommentRoot.POST.toString(), content: "test content", parentPost: postID});
    const childCommentDto = await createComment(postAuthor._id.toString(), {commentRoot: CommentRoot.POST.toString(), content: "test content", parentComment: parentCommentDto.comment._id, parentPost: postID});
  ...
  it('likeComment -- happy path', async () => {
    ...
    const post = await createTestPost(postAuthor);
    const postID = post._id.toString();
    const comment = await createComment(postAuthor._id.toString(), {commentRoot: CommentRoot.POST.toString(), content: "test content", parentPost: postID});
    await likeComment(postAuthor._id.toString(), {commentID: comment.comment._id});
    // TODO
  });

  it('likeComment -- user already likes it', async () => {
    // TODO
  });

  it('unlikeComment -- happy path', async () => {
    // TODO
  });

  it('unlikeComment -- user didnt like it before', async () => {
    // TODO
  });
});
